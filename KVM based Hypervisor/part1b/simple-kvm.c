...
/* count no. of times VM_EXIT is triggered */
uint32_t timesExited = 0;
/* count no. of times VM_EXIT is triggered for IO_IN event */
uint32_t timesExitedForIOIN = 0;
/* count no. of times VM_EXIT is triggered for IO_OUT event */
uint32_t timesExitedForIOOUT = 0;
/* to store gva address string address */
uint32_t gvaAddressString;
/* to store gva address */
uint32_t gva;
...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			// number of times VM_EXIT event is triggered
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xE6) {
				char *p = (char *)vcpu->kvm_run;
				uint32_t *data = (uint32_t*)(p + vcpu->kvm_run->io.data_offset);
				*data = timesExited;
				continue;
			}
			
			// print string hypercall 
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEB) {
				char *p = (char *)vcpu->kvm_run;
				uint32_t data;
				memcpy(&data, p + vcpu->kvm_run->io.data_offset, sizeof(uint32_t));
				uintptr_t addr = (uintptr_t)data;
				printf("%s", vm->mem + addr);
				fflush(stdout);
				continue;
			}
			
			// get gva for writing string back to guest os
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xE8) {
				char *p = (char*)vcpu->kvm_run;
				memcpy(&gvaAddressString, p + vcpu->kvm_run->io.data_offset, sizeof(uint32_t));
				continue;
			}
			
			// send count of IO_IN and IO_OUT event in specified format
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xE7) {
				char buffer[200];
				snprintf(buffer, sizeof(buffer),"IO in: %d\nIO out: %d\n", timesExitedForIOIN, timesExitedForIOOUT);
				memcpy(vm->mem + gvaAddressString, &buffer, strlen(buffer)+1);
				char *p = (char *)vcpu->kvm_run;
				void *address = (void*)(p + vcpu->kvm_run->io.data_offset);
				memcpy(address, &gvaAddressString, sizeof(gvaAddressString));
				continue;
			}
			...
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEC) {
				char *p = (char*)vcpu->kvm_run;
				memcpy(&gva, p + vcpu->kvm_run->io.data_offset, sizeof(uint32_t));
				continue;
			}

			// do gva to hva translation and return output to guest os
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xED) {
				char *p = (char*)vcpu->kvm_run;
				// translate gva to hva here
				struct kvm_translation tr;
				tr.linear_address = (uint64_t)gva;
				if(ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &tr) == -1) {
					perror("HC_gvaToHva: ioctl failed");
					continue;
				}
				// check if hva is valid for given gva
				if(!tr.valid) {
					printf("Invalid GVA\n");
				} else {
					void *returnAddress = (void*)(p + vcpu->kvm_run->io.data_offset);
					uint32_t hva = (uint64_t)(uintptr_t)(vm->mem + tr.physical_address);
					memcpy(returnAddress, &hva, sizeof(hva));
				}
				continue;
			}
