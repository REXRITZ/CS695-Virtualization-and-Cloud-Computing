...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    int TOTAL_VALUES = 3;
    int consumedValues[TOTAL_VALUES];
    /* switch VM every time ind == 2 indicating either 3 consumed values or 3 produced values */
    int ind = -1;
    int readyToConsume = 0;
    ...
    while (1)
    {
        ...
        case KVM_EXIT_IO:
            ...
            ind = (ind + 1) % TOTAL_VALUES;

            switch(vm->vcpus->kvm_run->io.port) {
                case 0x11:
                    int *x = (int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset);
                    /* doing ind-- because guest2b IO event is scheduled twice */
                    *x = consumedValues[ind--];
                    break;
                case 0x12:
                    ...
                    break;
                case 0x10:
                    consumedValues[ind] = *(int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset);
