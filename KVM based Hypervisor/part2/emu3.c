...
uint32_t *guestConsumerArrayAddress;
uint32_t *guestProducerArrayAddress;
uint32_t ind = 0;

void producePhase(struct vm *vm, struct vcpu *vcpu) {
	while(1) {
		...
		case KVM_EXIT_IO:
			/* read base address of producer array */
			...
			{ 
				...
				guestProducerArrayAddress = (uint32_t*)(vm->mem + addr);
				// printf("Producer address received\n");
				// fflush(stdout);
				continue;
			}

			/* write values produced by producer into consumer array */
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEB)
			{
				uint32_t *producer = guestProducerArrayAddress;
				uint32_t *consumer = guestConsumerArrayAddress;
		...
		halt:
			// printf("halt found vm1\n");
			break;
	}
}

void consumePhase(struct vm *vm, struct vcpu *vcpu) {
	while(1) {
		if (ioctl(vcpu->vcpu_fd, KVM_RUN, 0) < 0)
		{
			perror("KVM_RUN");
		...
		case KVM_EXIT_IO:
			...
			{ 
				...
				uint32_t addr;
				memcpy(&addr, p + vcpu->kvm_run->io.data_offset, sizeof(addr));
				guestConsumerArrayAddress = (uint32_t*)(vm->mem + addr);
				// printf("Consumer address received\n");
				// fflush(stdout);
				continue;
			}

			/* write values produced by producer into consumer array */
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xED)
			{
				printf("VMFD: %d Consumed Values: ", vm->vm_fd);
				for(int i = 0; i < TOTAL_ITEMS; ++i) {
					printf("%d ", guestConsumerArrayAddress[i]);
				}
				printf("\n");
				continue;
			}
...
int run_vm(struct vm *vm1, struct vm *vm2, struct vcpu *vcpu1, struct vcpu *vcpu2, size_t sz)
{
	...
	for (;;)
	{
		producePhase(vm1, vcpu1);
		consumePhase(vm2, vcpu2);
	}
