...
update_layer_stack() {
    ...
    if [ -z "$layer_stack" ]; then
        ...
        # TODO: is commented order correct?? If yes then revert back
...
handle_run() {
    ...
    mkdir -p $CACHEDIR/layers/$layer_hash/upper
    ...
    mount -t overlay overlay -o lowerdir="$parent_layers",upperdir="$CACHEDIR/layers/$layer_hash/upper",workdir="$CACHEDIR/layers/$layer_hash/work" "$CACHEDIR/layers/$layer_hash/rootfs"
    

    # Subtask 3.f.3
    # Execute the command in the new mount
    chroot "$CACHEDIR/layers/$layer_hash/rootfs" /bin/bash -c "$command"
    
    # Subtask 3.f.4
    # Cleanup and record metadata

    # unmount overlay fs here a.k.a cleanup
    umount "$CACHEDIR/layers/$layer_hash/rootfs"
...
handle_copy() {
    ...
    cp -a "$src" "$CACHEDIR/layers/$layer_hash/rootfs/$dest"
...
build() {
    ...
    while IFS= read -r instruction; do
        ...
        LAYER_STACK=$(update_layer_stack "$current_layer/upper" "$LAYER_STACK")
...
run() {
    ...
    mkdir -p "$CONTAINERDIR/$NAME/rootfs" # merged directory
    local LOWER_DIR=$(cat "$IMAGEDIR/$IMAGE/layers")

    mount -t overlay overlay -o lowerdir="$LOWER_DIR",upperdir="$CONTAINERDIR/$NAME/upper",workdir="$CONTAINERDIR/$NAME/work" "$CONTAINERDIR/$NAME/rootfs"
    # Subtask 3.d.2 - end
    ...
    unshare --fork --pid --uts --net --ipc --mount --kill-child chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "
...
exec() {
    ...
    nsenter --all --target="$CONTAINER_INIT_PID" -r -w sh -c "exec $EXEC_CMD_ARGS"
...
addnetwork() {
    ...
    # reference - https://manpages.debian.org/stretch/iproute2/ip-link.8.en.html
    ...
    ip -netns "$NAME" link set "$INSIDE_PEER" up
