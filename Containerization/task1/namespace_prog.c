...
int main() {
    ...
    */
    ...
    if(child_pid == -1) {
        errExit("clone failed for child1");
    }
    ...
    */
    ...
    /* wait to let child1 finish up creating namespace */
    sleep(1);

    char pidNamespacePath[100], utsNamespacePath[100];
    snprintf(pidNamespacePath, 100, "/proc/%d/ns/pid",child_pid);
    snprintf(utsNamespacePath, 100, "/proc/%d/ns/uts",child_pid);

    int pidFd, utsFd;
    /* pid symbolic link */
    if((pidFd = open(pidNamespacePath, O_RDONLY)) == -1) {
        errExit("child1 pid namespace descriptor fetch failed");
    }
    /* uts symbolic link */
    if((utsFd = open(utsNamespacePath, O_RDONLY)) == -1) {
        errExit("child1 uts namespace descriptor fetch failed");
    }

    /* join pid namespace -> only affects new child process created */
    if (setns(pidFd, CLONE_NEWPID) == -1) {
        errExit("pid setns failed");
    }
    ...
    if (fork() == 0) {
        ...
        */
        ...
        /* join uts namespace */
        if (setns(utsFd, CLONE_NEWUTS) == -1) {
            errExit("uts setns failed");
        }
